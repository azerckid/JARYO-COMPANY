...
- Acceptance Criteria:
  - [x] 레거시 서브시스템이 단계적으로 제거되며 각 단계에서 tsc/lint/test가 통과한다 (Slice 1b: 207파일 1376건 통과, tsc/eslint/build 클린)
  - [x] v1 현행 기능(자료수집·기장·부가세·급여·신고지원·직원명부·리마인드·신고준비)에 영향이 없다 (direct-upload+lib/upload 68건 재확인, 공유 API 미변경)
  - [ ] `clients`(사업장)·`billing`·jaryo-admin 등 유지 대상은 보존된다 (Slice 3~4에서 최종 확인)
- Document Sync Check: 2026-07-05 영향 감사 + Slice 1(Route Quarantine) + Slice 1b(죽은 컴포넌트/lib 삭제 + 포털 전용 API 4개 410 차단) 완료. `upload_session`은 현행 source lineage FK라 즉시 삭제 금지 원칙 유지. 공유 API(`/api/upload`·`/api/upload/submit`·`/api/upload/files/[fileId]`+retry/password)는 계속 미변경. `lib/session.ts`의 `getPortalData`+헬퍼 8개(~350줄) 정리는 블라스트 반경이 커서 의도적으로 별도 slice로 연기(2026-07-05 결정). 다음: (a) `session.ts` dead code 정리 slice, 또는 (b) Slice 2(메일 은퇴, `outbound_email`/request-event 신규 생성 경로 차단).
