...
- Implementation Preconditions:
  ...
  - [x] D-day 트리거 의미 확정 — daily_digest=매일, deadline_offset=마감−offsetDays 당일, manual=cron 제외([Brief §4](../03_Technical_Specs/14_INTERNAL_REMINDER_CRON_PRE_CODE_BRIEF.md))
...
- Acceptance Criteria:
  ...
  - [ ] daily_digest=매일 / deadline_offset=마감−offsetDays 당일 / manual=cron 제외로 due가 판정된다.
...
- Implementation Preconditions (조사·설계 과제):
  - [ ] 대상 세목 우선순위 확정 — 규격 공개본이 있는 지급명세서·원천세부터인지, 부가세 포함인지
  - [ ] 세목별 홈택스 전자신고 파일 규격 입수 (JC-023 Research §2.1의 [미확인] 규격서 공식 입수 경로 해소)
