...
| JC-030 | todo | 전자신고 검증 및 파일 생성 (Validation / Path 1a·1b) | `lib/efiling-*`, JC-024·013 | **최우선 — Path 1 세목 확대.** 자료대조 Phase 2와 간이지급 파일(1a) 후보 구현 완료. **원천세는 공식 양식 미확인 → Path 1b(직접입력 정리) 화면 구현 완료; 부가세도 Path 1b 대상으로 Field Mapping·Preview 오너 승인을 완료했고 Pre-Code Brief·runtime 구현 대기.** 공식 비암호화 전체 신고 양식이 확인되면 해당 세목만 1a(파일)로 승격하고, 그전에는 `항목 = 값` 직접입력 정리 대상으로 둔다(`blocked` 세목 없음). Stage A 통과 전 generator를 만들지 않는다. Path 3 암호화 파일은 범위 밖. [Path 1 Roadmap](../03_Technical_Specs/36_PATH1_FORM_FILL_ROADMAP.md) · [VAT Stage A Audit](../03_Technical_Specs/43_JC030_VAT_NONENCRYPTED_UPLOAD_TEMPLATE_AUDIT.md) |
...
- Implementation Preconditions (조사·설계 과제):
  ...
  - [ ] **부가세 1b runtime 직접입력 정리 화면** — 승인 Preview와 동일한 scoped read model로 확정 VAT 값을 법정 신고서 행·칸에 제공
...
- Document Sync Check Update (2026-07-13): **부가세 1b Field Mapping·UI-First Preview 오너 승인 완료.** 국세청 일반과세자 부가가치세 신고서 법정 행 번호와 현재 확정 VAT fact를 연결하고, 직접값·자동합계·홈택스 최종확인을 한 표로 분리했다. 현재 `payableTaxKrw`는 신고서 ㉰ 소계이므로 `(27)` 최종 납부(환급)세액으로 표시하지 않는다. Pre-Code Brief와 runtime read model·화면은 미완료다.
