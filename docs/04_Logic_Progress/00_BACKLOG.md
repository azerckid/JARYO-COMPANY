...
| JC-012 | done | Build payroll workspace | `lib/payroll-workspace`, `app/(dashboard)/dashboard/payroll`, `app/api/payroll` | Payroll register with derived totals, withholding/4-insurance deduction, insurance notice upload/manual confirmation, payslip/statement preview, and close guard match approved 급여 UI; PII raw fields/storage keys not exposed. **원천세 구성 팝오버(2026-07-12)**: 급여대장 원천세 금액(직원별 행·합계 행) 클릭 시 소득세·지방소득세 구성과 합계를 보여준다. 표시값은 모두 업로드된 급여자료 확정값이며(`withholdingTaxKrw = incomeTaxKrw + localIncomeTaxKrw`), 간이세액표로 재계산하지 않는다. 지방소득세는 위택스 별도 신고임을 명시. hover 단독이 아닌 클릭 팝오버(접근성). **정규직 소득세 간이세액표 직접 조회 전환(2026-07-12)**: 정규직 소득세를 하드코딩 값에서 근로소득 간이세액표(소득세법 시행령 별표2) 조견표 실제 공표 세액의 직접 조회값으로 전환(`lib/payroll/simplified-tax-table.ts`). 파생 산식이 아니라 `과세소득 구간 × 공제대상가족수` 조회라 법령 원문과 1:1 대조 검증 가능. `payroll_employee_line.dependent_count` 추가(마이그레이션 0072). 팝오버 출처 문구를 고용형태별로 분리(정규직=간이세액표, 프리랜서=3.3%, 일용직=일용 산식). 일용직 직원명부 4대보험을 `not_applicable`→`enrolled`(고용보험만)로 정정하고, 고용형태 기준 4대보험 정합성 교차검증(`lib/payroll-workspace/consistency.ts`)을 추가. 상세: [49_SIMPLIFIED_TAX_TABLE_LOOKUP](../03_Technical_Specs/49_SIMPLIFIED_TAX_TABLE_LOOKUP.md). |
...
| JC-043 | in_progress | 대화형 세무 작업공간 — 첫 화면·오케스트레이션 | 기존 회사 홈·자료수집·워크스페이스·AI orchestration | **다음 Gate: S2 첫 세로 여정 Preview 오너 확인.** [Preview 20](../02_UI_Screens/previews/20_sebiseo_first_vertical_journey.html) · [Prototype Review](../02_UI_Screens/19_SEBISEO_FIRST_VERTICAL_JOURNEY_PROTOTYPE_REVIEW.md) · [Execution Plan](./01_EXECUTION_PLAN.md) · [Operating Model](../01_Concept_Design/05_SEBISEO_OPERATING_MODEL.md) · [Brief 63](../03_Technical_Specs/63_JC043_CUI4_SEBISEO_UPLOAD_RESULT_CARD_PRE_CODE_BRIEF.md) · [QA 13](../05_QA_Validation/13_JC043_CUI4_SEBISEO_UPLOAD_RESULT_CARD_TEST_SCENARIOS.md) · [Preview 19](../02_UI_Screens/previews/19_sebiseo.html) |
...
- Implementation Preconditions:
  ...
  - [x] HTML Preview 동기화
  - [x] 기존 runtime route redirect/alias 정책 확정 — **URL을 하나도 바꾸지 않는다.** 사이드바의 시각적 그룹핑만 재배치하고 기존 라우트(`/dashboard/filing-support`, `/dashboard/filing-preparation/*`, `/dashboard/employees` 등)는 그대로 유지해 redirect 자체가 불필요하다.
  - [x] 실제 sidebar/breadcrumb 대상 파일과 회귀 테스트 범위 확정 — `sidebar.tsx`(신규 `PAYROLL_CHILD_NAV`/`ANNUAL_FILING_FIXED_CHILD_NAV`), `layout.tsx`(`loadPrimaryBusinessEntityType` 추가), breadcrumb 15개 파일, `sidebar.test.ts`
- Acceptance Criteria:
  - [x] runtime 사이드바에 신고지원·신고 준비 상위 메뉴가 없다.
  - [x] 급여·지급 하위 메뉴와 parent/child active 상태가 정확하다.
  - [x] 법인은 법인세, 일반 개인은 종합소득세, 면세 개인은 종합소득세·사업장현황신고만 본다. 미지정(unknown)은 하위 메뉴 없이 상위 항목만 노출한다(과잉 추정 방지).
  - [x] 회사 홈의 다가오는 신고가 가장 가까운 일정 2~3건과 해당 세목 CTA(`준비 화면 열기`)를 제공한다.
  - [x] 기존 URL은 redirect/alias로 안전하게 유지되고 직접 링크 회귀가 없다(URL 미변경).
- Document Sync Check (2026-07-11): `lib/tax-calendar.ts`에 `buildUpcomingSchedule`/`UpcomingScheduleItem`(+ 세목별 `href` 매핑)을 두어 `lib/filing-preparation/summary.ts`와 `lib/company-home/summary.ts`가 공유(순환 import 회피, 원래 `filing-preparation/summary.ts`에 있던 함수를 이전 후 재-export). `filing-preparation-hub.tsx`의 "다가오는 세무 일정" 섹션은 제거하고 breadcrumb을 "연간신고"로 갱신. tsc/lint(0 errors)/vitest(239 files·1664 tests) 전체 통과, whitespace clean. 브라우저 검증: 회사 홈 사이드바·다가오는 신고 스트립·CTA 라우팅, 급여·지급/원천세/연간신고 breadcrumb, dev DB에서 `taxEntityType`을 corporation/tax_exempt로 임시 전환해 연간신고 조건부 하위 메뉴(법인세 단독 / 종합소득세+사업장현황신고) 확인 후 원상복구(null) 완료. 검증 중 `/dashboard/settings` 사업자 유형 저장 시 발생한 별도 pre-existing 버그(`{formErrors, fieldErrors}` 객체를 React child로 직접 렌더 — `settings-panel.tsx`/`/api/settings/tenant`)를 발견했으나 JC-036 변경 파일과 무관해 이번 범위에 포함하지 않음(후속 이슈로 별도 처리 필요).
...
- Implementation Preconditions:
  ...
  - [x] 자료대조원장 Slice 2a-5 full read model wiring 완료 — Brief §2.1 step 2a-5, PR #161·#162·#163·#164 merged(`ae2a94b`). 사이드바 기본 경로가 live 데이터로 전환됨(`display=fixture` 명시할 때만 fixture)
  - [ ] 자료대조원장 기간 모드 연/사용자 지정 실제 구현 — 2a-5 범위에서 제외, 월/분기/반기만 우선 지원(`buildCompanyHomePeriod`), 후속 작업으로 별도 진행
  - [x] 자료대조원장 explanation_required·explained_no_evidence·evidence_exception 상태 실 데이터 도출 — 개인/업무무관 보수적 휴리스틱과 `staffMemo` v1 규약으로 구현. 전용 enum/컬럼은 감사 요구가 커질 때만 재검토
  - [ ] `ReconciliationLedgerDisplayFixtureView`/`reconciliation-ledger-display-fixture-view.tsx` 이름 정리 — live 데이터도 렌더하는데 이름에 Fixture가 남아있음, 동작엔 문제없어 비차단 후속 작업
  ...
  - [x] 자료대조원장 편의성 계약 검토 — table-first 기간/행동 필터, 계정 패턴 일괄 수락, 행 한 줄 결론, 자료수집 역링크, 세목별 차단 이유, 마감 체크리스트, 최근 적용 취소. PR #171에서 중복 hero/source summary/다음 할 일 카드는 시각 부담 때문에 제거
  ...
  - [x] 자료대조원장 Slice 2b-2c-2 증빙 예외 처리 구현 — `staffMemo`에 `증빙 예외: ...` 형식으로 저장하고 기존 `linkedEvidenceRowId`를 비움. 예외 처리된 행은 `증빙 예외` 칩으로 표시하고 증빙 찾기/확인 드롭다운을 닫음. 저장 후 현재 세션 undo 이후의 예외 해제·수정은 후속 과제
  - [x] 자료대조원장 Slice 2b-2c-3 금액 차이 표시·저장 차단 구현 — 거래처/일자가 비슷해도 금액이 다르면 `증빙있음`으로 올리지 않고 `증빙 필요`로 유지. 증빙 찾기 모달 안에서 `금액 차이`로 알려주며, 금액이 다른 행은 직접 연결 저장을 막음. 부분 지급·다건 합산을 확정 저장하는 편집기는 후속 과제
  - [x] 자료대조원장 Slice 2b-3a 계정 반복 패턴 적용·무시 구현 — 같은 거래처/방향의 과거 확정 계정 패턴을 `patternSuggestion.suggestedAccount`로 표시하고, 계정 팝오버에서 패턴 적용 또는 무시(`패턴 거부: ...` memo)를 처리
  - [x] 자료대조원장 Slice 2b-3b-1 증빙·제외 반복 패턴 표시 + 단건 확인 진입점 구현 — 같은 거래처/방향의 과거 증빙 연결·제외 처리 이력을 `patternSuggestion`으로 표시하고, 증빙 찾기 출처 강조·제외 모달 사유 prefill로 연결. 자동 연결·자동 제외 없음
  - [x] 자료대조원장 Slice 2b-3b-2a 계정 패턴 안전 일괄 수락 구현 — 동일 거래처·동일 근거·동일 계정 추천 그룹만 표시하고, 대상 행 확인 모달 + 현재 세션 undo 가능한 계정 확정 mutation으로 처리
...
- Acceptance Criteria:
  ...
  - [x] 자료대조원장에 입출금↔세금계산서/카드/현금영수증 대조·발견 결과와 실제 증빙 행이 표시된다. 금액 차이는 `증빙있음`으로 승격하지 않는다.
  ...
  - [x] 행 한 줄 결론과 모달 액션을 제공하고, 부족한 자료는 자료수집 역링크로 연결하며, 최근 적용 1건을 취소할 수 있다.
  - [x] 자료대조원장에서 계정항목 확정·사용내역 소명·업무무관/사적 사용 제외 사유가 한 화면 흐름으로 처리된다.
...
- Related HTML Preview: [09_payment_year_end.html](../02_UI_Screens/previews/09_payment_year_end.html) — 지급명세서, [15_year_end_settlement.html](../02_UI_Screens/previews/15_year_end_settlement.html) — 연말정산. 2026-07-13 별도 메뉴·화면으로 분리하고 신고 준비 허브의 공용 트랙/read model은 유지.
...
- Related UI Docs: [UI Design §4.11](../02_UI_Screens/01_UI_DESIGN.md) — 지급명세서 화면의 JC-030 직접작성 값 패널
- Related HTML Preview: [09_payment_year_end.html](../02_UI_Screens/previews/09_payment_year_end.html) — JC-030 Path 1b 직접작성 패널(재승인 방향 2026-07-13)
- Related QA Docs: [Filing Support Test Scenarios §2.10](../05_QA_Validation/07_FILING_SUPPORT_TEST_SCENARIOS.md) — 세목별 Path 1 공식 비암호화 양식·Preview·다운로드·실제 업로드 검증 완료선.
- Prototype Review / 승인: [x] — `09_payment_year_end.html` Path 1b 전환 방향 사용자 승인(2026-07-13)
- Implementation Preconditions (조사·설계 과제):
  ...
  - [x] 파일변환신고 적합성 검정·암호화 경로는 현재 제품 범위 밖으로 결정 — [Archived Research](../03_Technical_Specs/32_JC030_SW_CONFORMANCE_CERTIFICATION_RESEARCH.md)
  - [x] **UI-First Gate 재판정**: [09_payment_year_end.html](../02_UI_Screens/previews/09_payment_year_end.html) 간이지급 Path 1b 직접작성 패널 — 사용자 방향 승인(2026-07-13)
  - [x] 간이지급 Path 1b read model·runtime — 홈택스 경로, 사업자/기간, 소득자별 근무기간·6개월 지급액·지급총액·인정상여
  - [x] 과거 plain 후보 파일·PII 입력·다운로드·변환제출 UI 제거 — 기술 검증 builder/API만 비노출 보존
  ...
  - [x] **부가세 Stage A 공개 자료 감사** — 현재 홈택스 `파일 변환신고(회계프로그램)` 메뉴와 국세청 v1.51 일부 첨부서류 Excel 도구 확인. 전체 신고 비암호화 규격과 2026년 수용 여부는 미확인 → 부가세는 **Path 1b 대상으로 결정**([43](../03_Technical_Specs/43_JC030_VAT_NONENCRYPTED_UPLOAD_TEMPLATE_AUDIT.md))
  - [x] **부가세 1b Field Mapping + HTML Preview** — 일반과세자 정기 확정신고 법정 행·칸과 확정 VAT fact의 대응, 미지원 행, ㉰와 `(27)` 경계를 고정하고 별도 `홈택스 입력값` Preview 제작([52](../03_Technical_Specs/52_JC030_VAT_PATH1B_FIELD_MAPPING.md) · [Preview](../02_UI_Screens/previews/14_vat_path1b.html))
  - [x] **부가세 1b UI-First Gate 오너 승인** — desktop/mobile Preview의 표 구조·문구·책임 경계 확인(2026-07-13)
  - [x] **부가세 1b runtime 직접입력 정리 화면** — 승인 Preview와 동일한 scoped read model로 확정 VAT 값을 법정 신고서 행·칸에 제공. blocked/empty/stale/unsupported에서는 과거 값 숨김, `(27)`은 홈택스 최종 확인, AI·파일 generator 없음
  - [ ] **부가세 Stage A 외부 확인(1a 승격용·선택)** — 로그인 화면 또는 126으로 전체 신고/부속명세의 최신 원본·버전·비암호화 직접 수용 여부 확인
  - [ ] **부가세 1a Path B~G** — Stage A가 양식을 확인한 뒤에만 Phase 2 gate/provenance를 소비하는 공식 업로드 파일 구현
  - [x] **지방소득세 Stage A-1 공개 근거 감사** — 위택스 공식 매뉴얼에서 `엑셀파일신고`, 공식 서식 `B070101-02.xlsx`, 업로드 검증·제출·전자납부번호 확인 절차를 확인. 민간 `회계파일신고`·비밀번호 경로는 제외([54](../03_Technical_Specs/54_JC030_LOCAL_INCOME_TAX_UPLOAD_TEMPLATE_ACQUISITION.md))
  - [ ] **지방소득세 Path 1a A~G** — 로그인 위택스에서 공식 원본을 입수해 hash·시트·열·유효성을 고정한 뒤 Mapping·Preview·오너 승인·Pre-Code·generator·실제 위택스 수용 검증 순서로 진행. 원본 전 1b 화면·임의 workbook·generator 구현 금지
  - [x] **근로소득 지급명세서 Stage A 공식 경로 감사** — 국세청 공식 2025 안내에서 홈택스 직접작성제출 또는 자체 프로그램 변환제출을 확인. 공식 비암호화 업로드 양식은 확인되지 않았고 법정 HWP는 업로드 양식이 아님 → **Path 1b 대상**([55](../03_Technical_Specs/55_JC030_ANNUAL_WAGE_STATEMENT_STAGE_A_AUDIT.md))
  - [x] **근로소득 지급명세서 Stage B Field Mapping** — 현행 별지 제24호서식(1)의 법정 필드를 canonical source·canonical 확장·홈택스 직접입력·신뢰 source 필요·v1 제외로 분류했다. 현재 모델에는 연간 지급·기납부세액 외 공제·과세표준·결정세액·환급/추징 정본이 없어 UI-First Gate를 NO-GO로 판정했다([56](../03_Technical_Specs/56_JC030_ANNUAL_WAGE_STATEMENT_FIELD_MAPPING.md))
  - [x] **근로소득 지급명세서 Stage C canonical source 계약** — 홈택스 편리한 연말정산 생성 결과를 최종 정본으로 두고, SemuAgent는 확정 급여 기초자료 준비만 담당한다. 신고 사업장 profile ownership과 주민등록번호·부양가족·공제증빙·최종 결과 문서의 비수집·비저장 경계를 확정했으며, 세액 계산·역산·AI 생성·결과 import는 v1에서 제외했다([57](../03_Technical_Specs/57_JC030_ANNUAL_WAGE_STATEMENT_CANONICAL_SOURCE_CONTRACT.md))
  - [x] **근로소득 지급명세서 Stage D HTML Preview 제작** — [연말정산 Preview](../02_UI_Screens/previews/15_year_end_settlement.html)에 간결한 기초자료 준비 화면, 홈택스 생성 흐름, 직원별 `급여 준비 완료/급여 보완/특례 확인` 상태와 행 상세를 반영했다. runtime·schema·API 변경 없음
  - [x] **근로소득 지급명세서 Stage D 오너 확인** — 직원 표의 정보 밀도·상태 3종·행 상세 구조 승인(2026-07-14)
...
- Per-tax Done: 세목마다 두 경로 중 하나로 완결한다. **Path 1a(양식 있음):** [Roadmap §2.1](../03_Technical_Specs/36_PATH1_FORM_FILL_ROADMAP.md)의 공식 비암호화 업로드 양식·매핑·UI/Brief·동일 read model·파일 형식·브라우저·실제 업로드 검증·문서 동기화를 모두 통과. **Path 1b(양식 없음):** 확정값을 `항목 = 값`으로 정리한 직접입력 화면을 **구현**(파일 generator·업로드 검증 없음). 세목 매트릭스 의사결정은 §2.1 통과(1a) 또는 공식 Stage A 근거로 양식이 없어 1b 대상 확정으로 닫으며, **`blocked` 세목은 없다.** 간이지급명세서·원천세·부가세는 1b 값 정리 화면 구현·검증을 완료했고(각 2026-07-13·2026-07-12·2026-07-13), 나머지 1b 대상 세목은 미구현이다. Path 1a 베타 세목 수에는 1a 완료 세목만 포함한다.
- Document Sync Check (2026-07-13): **간이지급명세서는 Path 1b 직접작성으로 재판정·구현 완료.** 공개 국세청 안내는 직접작성 제출을 제공하지만 고정길이 plain 후보를 직접 수용하는 공식 비암호화 양식·메뉴를 확인하지 못했다. 사용자 화면에서 파일 후보·PII 입력·다운로드·변환제출 안내를 제거하고, 홈택스 경로와 확정 급여의 월별 지급액·합계를 제공한다. 과거 builder/API는 비노출 기술 검증 자산으로만 보존한다. 원천세와 부가세도 1b 구현·검증을 완료했다. Path 1a 완료 세목은 아직 없으며, 공식 양식과 실제 직접 수용 검증이 확인된 세목만 1a로 센다. Path 2는 전체 Path 1 베타 이후, Path 3 암호화 파일은 범위 밖이다.
...
- Document Sync Check Update (2026-07-13): **부가세 1b Field Mapping·UI-First Preview·Pre-Code·runtime 구현·검증 완료.** `/dashboard/vat/hometax-input`은 같은 tenant·사업장·기간의 검증된 VAT fact와 공제 검토·package/provenance gate를 읽는다. ready에서만 법정 행 번호·금액·세액·확인 방식을 한 표로 표시하고, blocked/empty/stale/unsupported에서는 과거 값을 숨긴다. 현재 `payableTaxKrw`는 신고서 ㉰ 소계로만 표시하며 `(27)`은 홈택스 최종 계산값으로 남긴다. AI provider·파일 생성·자동입력·자동제출은 없다. 전체 257파일 1,806건·tsc·lint·whitespace·문서 링크 검증과 로그인 desktop/410px browser 검증을 통과했다.
- Document Sync Check Update (2026-07-14): **지방소득세 특별징수는 Path 1b가 아니라 Path 1a 후보로 재판정했다.** 위택스 현재 공식 매뉴얼에서 직접 배포하는 `B070101-02.xlsx`와 `엑셀파일신고`의 다운로드→작성→업로드 검증→제출 흐름을 확인했다. 민간 회계프로그램 파일·암호/비밀번호 경로는 범위 밖이다. 로그인 원본을 아직 입수하지 않았으므로 Stage B Mapping, Preview, runtime, generator는 시작하지 않으며 원본 hash·시트·열·유효성 고정이 다음 완료선이다.
- Document Sync Check Update (2026-07-14): **근로소득 지급명세서 Stage A를 완료하고 Path 1b 대상으로 판정했다.** 국세청 공식 안내의 직접작성제출과 자체 프로그램 변환제출을 구분했고, 공식 비암호화 업로드 양식은 확인되지 않았다. 현재 연말정산 read model에는 연간 지급·기납부세액만 있어 전체 지급명세서 값을 만들 수 없으므로 다음은 Stage B 법정 필드·canonical 데이터 공백 매핑이다. 사업장현황신고는 면세 개인사업자 조건부 후순위로 변경했다.
- Document Sync Check Update (2026-07-14, Stage B 시점): **근로소득 지급명세서 Stage B Field Mapping을 완료하고 UI-First Gate를 NO-GO로 판정했다.** 현행 소득세법 시행규칙 별지 제24호서식(1)의 사업자·근무처·급여/비과세·세액·소득공제·세액공제·결정세액·기납부세액·차감징수세액 필드를 전부 ownership별로 분류했다. 현재 DB의 `annualGrossPayKrw`·`annualWithholdingTaxKrw`와 월 급여 라인만으로는 완전 신고서 값을 만들 수 없고, billing profile을 신고 정본으로 재사용하거나 공제·결정세액을 역산하지 않는다. 당시 다음 게이트는 Stage C 정본·신고 profile·PII 계약이었고, 바로 아래 Stage C 업데이트에서 완료 상태를 기록한다.
- Document Sync Check Update (2026-07-14, Stage C 시점): **근로소득 지급명세서 Stage C canonical source 계약을 완료했다.** 국세청 편리한 연말정산은 회사 기초자료와 근로자 공제신고서를 결합해 지급명세서를 생성하고, 회사가 확인·수정 후 별도로 제출한다. SemuAgent는 확정 급여 기초자료 준비만 담당하고 최종 공제·결정세액·차감징수세액은 홈택스 생성 결과를 정본으로 둔다. 청구 profile은 신고 profile로 재사용하지 않으며 주민번호·가족·공제증빙·최종 PDF/HWP를 수집·저장하지 않는다. 당시 다음 게이트는 Stage D HTML Preview와 오너 승인이고, 승인 전 runtime·schema·API를 만들지 않는 것으로 고정했다.
- Document Sync Check Update (2026-07-14, Stage D Preview 시점): **근로소득 지급명세서 Stage D HTML Preview를 제작했다.** 기존 `연말정산` 메뉴 아래 한 화면에서 홈택스 편리한 연말정산의 기초자료→공제신고서→지급명세서 생성→확인·수정→제출 흐름과 직원별 급여 준비값을 보여준다. 상태는 `급여 준비 완료/급여 보완/특례 확인` 3개만 사용하고, 최종세액·환급/추징·주민번호·공제증빙 입력·중복 설명 카드는 포함하지 않았다. 제작 시점에는 오너 확인 대기였고, 바로 아래 승인 업데이트에서 완료 상태를 기록한다.
- Document Sync Check Update (2026-07-14, Stage D Owner Approval): **프로젝트 오너가 Stage D Preview의 직원 표 정보 밀도·상태 3종·행 상세 구조를 승인했다.** 다음은 Stage E Screen Flow·UI Design·Pre-Code Brief 동기화이며, 해당 계약을 닫기 전 runtime·schema·API를 만들지 않는다.
- Document Sync Check Update (2026-07-14, Stage E Pre-Code): **Screen Flow·UI Design·Pre-Code Brief와 Stage C 계약을 승인 Preview 기준으로 동기화했다.** 기존 `payroll_period_summary`·`payroll_employee_line`·`employee_profile`에서 급여 항목·4대보험·기납부 소득세/지방소득세를 tenant·사업장·연도·직원 범위로 집계하고, 완료 연도 상태는 `급여 준비 완료/급여 보완/특례 확인` 세 종류로 고정했다. 진행 중 연도는 별도 상태이며, 현재 DB에 없는 특례를 `해당 없음`으로 단정하지 않는다. 첫 runtime에는 `client_filing_profile`·신규 migration·mutation API·AI가 필요 없다고 결정했다. 당시 다음 단계는 Stage F runtime·browser QA·closeout이었다.
- Document Sync Check Update (2026-07-14, Stage F Runtime Closeout): **근로소득 지급명세서 Path 1b runtime을 승인 Preview 구조로 교체했다.** 월별 급여 기간과 직원 line이 모두 마감된 경우만 급여·보험·기납부세액을 연간 집계하고, `급여 보완 > 특례 확인 > 급여 준비 완료` 우선순위를 적용한다. 진행 중 연도는 완료 상태에 섞지 않으며, 직원 행 상세에서 홈택스 직접 확인 책임을 분리한다. 단위·정적·desktop/mobile 브라우저 QA와 샘플 마감상태 생성 규칙 정합화를 완료했고 신규 DB·API·AI·PII 수집은 없다.

### JC-035 · 부가세 AI 세무판단 보조 — 공제·과세유형 판단 작업대 (완료)

- Status: `done` — VAI-0~6b 구현·머지(PR #200)·dev/prod migration `0070`·브라우저 E2E 완료
- Related Domain: JC-011 부가세 · JC-010 자료대조원장 · exact VAT fact · `vat_deduction_review` · VAT provenance/rebuild
- Related Completion Contract: [VAT AI Tax Treatment Completion Contract](../03_Technical_Specs/44_VAT_AI_TAX_TREATMENT_COMPLETION_CONTRACT.md) — VAI-0~6b 고정 순서·완료선·AI 실패 안전성
...
- Related QA Docs: [VAT Test Scenarios §2.10](../05_QA_Validation/05_VAT_TEST_SCENARIOS.md) — 규칙/패턴/AI/consensus/fallback/사용자 확정
- Prototype Review / 승인: [x] — 기존 JC-011 화면 승인 유지, JC-035 AI 판단 작업표 프로젝트 오너 승인
- Product Boundary:
  - AI는 공제/불공제/안분과 과세/영세율/면세의 가능성·근거·필요 증빙을 제시한다.
  - 최종 판단은 사용자이며, 사용자 확정 전 VAT fact·공제 decision·세액·package gate를 바꾸지 않는다.
  - 공식 업로드 파일 생성(1a)·직접입력 정리 화면(1b)은 JC-030 트랙이며 JC-035 범위 밖이다.
  - 단계별 직접입력 가이드·자동제출·자동납부·세무대리·AI 자동확정은 포함하지 않는다.
- Implementation Preconditions:
  - [x] 현재 exact VAT fact·공제검토·provenance·AI orchestration 재사용 가능성 감사
  - [x] VAI-0 완료 계약·판단 단계·timeout/fallback·tenant 격리·비목표 고정
  - [x] VAI-1 HTML Preview 작성 — 판단 가능성·근거·필요 증빙·사용자 확정·AI 실패 상태
  - [x] VAI-1 프로젝트 오너 브라우저 확인·승인
  - [x] VAI-2 공식 규칙 매트릭스 작성 — 조문/시행령/국세청 안내 버전·적용일·필요 사실·필요 증빙
  - [x] VAI-2 별도 Pre-Code Brief 작성 — Zod/read model/API/mutation/audit/PII
  - [x] VAI-2 Rule Matrix·Pre-Code Brief 프로젝트 오너 승인
- Acceptance Criteria:
  - [x] deterministic 규칙·이전 확정 패턴·AI 보강·multi-provider 합의가 근거와 함께 구분된다.
  - [x] 영세율·면세는 필수 증빙이 없으면 확정 및 downstream gate 해제가 차단된다.
  - [x] 사용자가 영세율·면세 필수 증빙을 확인 완료로 기록하고 확인자·시각을 감사할 수 있다.
  - [x] AI timeout·quota·provider 오류에도 화면·수동 검토·기존 mutation이 비차단 동작한다.
...
- Current Gap:
  ...
  - VAI-8b 이전에는 어떤 자료를 실제로 찾았고 무엇을 찾지 못했는지 완전한 evidence trace가 없었다.
...
- Acceptance Criteria:
  ...
  - [x] 특례 근거가 없으면 해당 없음과 보수적 기본 방향을 제시한다.
  - [x] 낮은 confidence·단일 provider 실패·timeout만으로 담당자에게 넘기지 않는다.
  - [x] 담당자 이관은 허용 조건과 잠정 결론·탐색 자료·부족/충돌 근거·정확한 질문·결론 변경 조건을 모두 포함한다.
...
- Scope (1차, 상세는 Brief 51 참조):
  - 법정 불공제 사유 8개 중 **④ 접대비(기업업무추진비) 관련만** 1차로 다룬다(신뢰도 게이트가 아니라 기능 범위 경계). 재분류 후보는 복리후생비·회의비.
  - JC-039 evidence resolver를 재사용해 참석자·적요·과거 확정 이력에서 재분류 신호(가산·감산)를 찾는다(Brief 51 §4.1 factors table).
  - **핵심 원칙(v2): 후보는 넓게, 확정은 엄격하게.** 범위 안의 접대비 불공제 후보는 신뢰도(높음/중간/낮음)와 무관하게 항상 목록에 표시한다. 참석자 정보 없음·고액·법인 거래처명·과거 접대비 유지 이력은 후보 배제 조건이 아니라 신뢰도를 낮추는 감산 요인일 뿐이다. 각 후보는 "왜 후보인지"(factors)와 "무엇을 추가하면 공제 가능한지"(missingToConfirm)를 항상 함께 보여준다.
  - 절세 금액은 확정 표현이 아니라 가능성 표현으로 보여준다 — "OOO원 절세" 대신 "최대 OOO원 추가 공제 가능성"을 확정 조건·부족한 근거와 함께 표시(Brief 51 §6).
  - 실제 공제 전환(canonical 값 변경)은 별도의 엄격한 확정 게이트를 통과해야 한다 — 업무 목적, 적격증빙, 사용자 명시 확인 세 가지가 모두 충족되어야 하며 AI는 자동으로 전환하지 않는다(Brief 51 §5). "유지" 선택 시 같은 패턴에 재질문하지 않는다.
- Out of Scope (1차):
...
- Status: `in_progress` (CUI-4 Closeout·shell #286 · **S2 첫 세로 여정 Preview 오너 화면 확인 대기**)
...
- Related UI Docs: [Screen Flow §2.1](../02_UI_Screens/00_SCREEN_FLOW.md) · [S2 Prototype Review](../02_UI_Screens/19_SEBISEO_FIRST_VERTICAL_JOURNEY_PROTOTYPE_REVIEW.md) · [UI Design](../02_UI_Screens/01_UI_DESIGN.md)
- Related HTML Preview: [세비서 19](../02_UI_Screens/previews/19_sebiseo.html) · [첫 세로 여정 S2](../02_UI_Screens/previews/20_sebiseo_first_vertical_journey.html)
...
- Implementation Preconditions:
  ...
  - [x] 메뉴명 `세비서`, 사이드바 최상단·로그인 첫 화면, 회사 홈 유지 결정.
  - [x] CUI-1 HTML Preview(`19_sebiseo.html`) 제작·오너 반복 피드백 반영.
  ...
  - [x] CUI-3c runtime: 서버 고정 허용목록 기반 화면 이동 CTA(PR #269 머지)
  - [x] CUI-3d: T/U/C/R/I/N/P와 자료수집 S-60~S-64 통과. 전용 `semuagent-staging` DB + private `semuagent-preview` Blob에서 PDF/XLSX callback·DB 상태·자료수집 이동을 검증하고 QA 데이터는 전부 정리([QA 12 §9.2](../05_QA_Validation/12_JC043_CUI3_SEBISEO_TEST_SCENARIOS.md))
  ...
  - [x] CUI-4 Closeout: QA 13 R-01~R-12 Preview 실측 (#280~#284)
  - [x] 대시보드 상단 바·네비 hide·파일 카드 가로 배치 (#286)
  - [x] S2 첫 세로 여정 HTML Preview — [20](../02_UI_Screens/previews/20_sebiseo_first_vertical_journey.html) · [Prototype Review](../02_UI_Screens/19_SEBISEO_FIRST_VERTICAL_JOURNEY_PROTOTYPE_REVIEW.md) **오너 화면 확인 대기**
  - [ ] S2 HTML UI Preview Gate·UI-First Gate 오너 승인·피드백 기록
  - [ ] CUI-4d Brief Ready·승인 (S2 Gate 후 · #287 Draft)
...
- Acceptance Criteria:
  ...
  - [x] 업로드는 기존 tenant-scoped 자료수집 저장·파싱·정규화 경로를 재사용한다(PR #267).
...
- Document Sync Check (2026-07-17, CUI-3a): 세비서 기간 확인 UI·`staff_direct` 업로드 연결은 PR #267로 머지됐다. Instant/Mic/Voice는 비활성으로 유지한다.
- Document Sync Check (2026-07-17, CUI-3b): `POST /api/sebiseo/chat`, history≤8·2000자·client/server redaction·화이트리스트·tenant+user rate limit·ephemeral UI를 구현했다. 첫 로드/업로드 후 자동 provider 호출과 canonical mutation은 없고 Instant/Mic/Voice는 비활성 유지한다. PR #268로 머지, 로그인 브라우저 E2E(실제 AI 응답·3종 거절·429·모바일)로 검증했다.
- Document Sync Check (2026-07-17, CUI-3c): 허용된 답변 아래 화면 이동 CTA를 붙였다. 버튼은 `lib/sebiseo/chat/screen-actions.ts` 서버 고정 허용목록에서 사용자 메시지 키워드로 도출하며 AI 출력은 URL을 만들지 못한다. 명확한 세목은 상세 화면, 포괄 연간신고만 허브로 이동한다. 스키마 상한 3, 런타임 최대 2 노출. 거절·무-doc·오류 응답에는 버튼이 없고 클릭은 기존 `/dashboard/*` 이동만 한다(mutation·확정·신고 없음). href는 sidebar 라우트와 일치하도록 드리프트 가드 테스트로 고정했다.
- Document Sync Check (2026-07-17, CUI-3d): localhost에서 발화하지 않는 Vercel Blob callback을 production과 분리된 `semuagent-staging` DB·private `semuagent-preview` Blob·고정 staging URL에서 검증했다. PDF/XLSX `staff_direct` 업로드, H2 기간, callback, `analyzing/needs_review`, 자료수집 이동을 확인했다. production DB·환경변수는 무변경이며 staging/Blob QA 데이터는 0건으로 정리했다. CUI-3 완료, 다음은 CUI-4다.
- Document Sync Check (2026-07-17, CUI-4 Preview draft): 중립 첫 화면을 유지하고 `다른 상태 보기 · 업로드 결과` 안에 최근 세션 카드 1개만 추가했다. runtime은 현재 tenant·사업장·기간·session의 DB 상태에서 건수를 파생하고, CTA는 같은 `period + sessionId`로 좁힌 자료수집 화면만 열며 mutation·확정은 하지 않는 계약으로 오너 확인 대기다.
- Document Sync Check (2026-07-17, CUI-4 Preview approved): Preview `19_sebiseo.html`의 업로드 결과 카드 1개·접힌 예시·모바일 레이아웃을 오너가 승인했다(PR #271). runtime/API/DB 변경은 Pre-Code Brief 승인 전 착수하지 않는다.
...
- Document Sync Check (2026-07-19, S2 first vertical journey Preview): [20_sebiseo_first_vertical_journey.html](../02_UI_Screens/previews/20_sebiseo_first_vertical_journey.html)과 [Prototype Review](../02_UI_Screens/19_SEBISEO_FIRST_VERTICAL_JOURNEY_PROTOTYPE_REVIEW.md)를 #286 이후 main 기준으로 정리했다. 파일/거래 구분·필터 원장·확정 상태·세비서 재조회. DB/API/runtime·CUI-4d Brief 미포함. 오너 화면 확인 대기.
...
- Fixed Order:
  1. T0: Brief·QA 오너 승인, literal color audit, 필요 시 작은 token Preview. **완료(문서)**.
  2. T1: root ThemeProvider, System/Light/Dark selector, global/company/shadcn token completion, dashboard shell 및 overlay만 양 테마화. 세비서 literal은 T2 전까지 남을 수 있다(예상 중간 상태).
  3. T2: Sebiseo(고정 charcoal 제거)·회사 홈 -> 자료수집·기장/자료대조 -> 급여/지급 -> VAT·연간신고/설정 순으로 workspace 전환.
  4. T3: two-theme browser, mobile, keyboard, contrast, mutation-in-flight regression QA.
- Implementation Preconditions:
  - [x] System/Light/Dark, OS 기본값, Sidebar 하단 selector를 승인한다.
  - [x] browser-local preference만 사용하고 tenant/user DB setting은 만들지 않음을 승인한다.
  - [x] Sebiseo가 전역 테마를 따르고 dark-only literal palette를 없애는 것을 승인한다(제거 시점은 T2).
  - [x] 한 번의 global flip 대신 token foundation부터 화면군별로 전환하는 순서를 승인한다.
  - [x] public/sign-in surface는 첫 slice에서 제외하는 것을 승인한다.
- Acceptance Criteria:
...
- Implementation Preconditions:
  ...
  - [x] JARYO `수신 승인`은 전송 snapshot을 `승인된 수신자료`로 승격하는 것이며, 확정 장부·신고 정본 승격은 별도 승인으로 분리
  - [x] 수임 연결 승인 시 JARYO 기존 사업자를 선택하거나 신규 생성해 고유 연결 ID를 확정하고, 이후 활성 연결 ID로만 전송
  - [x] transfer 패키지는 부분 수신하지 않고 `수신 승인`·`보완 요청`·`기술 오류` 중 하나로 처리
  - [x] 동일 transfer/fingerprint 재시도는 멱등 처리하고, 보완 재전송은 새 snapshot version으로 등록한 뒤 수신 승인 시 이전 version을 대체 상태로 보존
  - [x] 수신 검토 문제를 사무소 세무 판단·사업자 자료/사실 보완·전송 기술 오류로 분리
  - [x] 사무소 변경은 예외로 취급하고 변경 전·후 값·사유 유형·실제 근거·담당자·시각을 기록
  - [x] JARYO AI가 구조화 변경정보로 사업자용 설명 초안을 작성하되 담당자 검토·승인 전 전송 금지
  - [x] 사무소 최종 조정은 SemuAgent 원본을 덮어쓰지 않고 별도 adjustment로 기록하며 해당 신고 결과의 최종값으로 표시
  - [x] 사무소 조정은 사업자 재승인을 기다리지 않고 알림·문의 경로를 제공하며 다른 기간·규칙으로 자동 확장 금지
  - [x] 보완 요청은 전송 후 새 사실·SemuAgent Ready 누락의 예외 복구로만 사용하고 정확한 자료 작업에 연결
  - [x] 보완 요청 시 관련 Ready 재잠금, SemuAgent 내 보완, 전체 gate 재검사, 사업자 확인 후 새 snapshot 재전송
  - [x] 동일 승인 snapshot·fingerprint의 일시적 기술 오류는 멱등 자동 재시도하고 내용 변경 시 Ready 재검사·사업자 재승인
  - [x] v1 신고 결과는 JARYO 담당자의 최소 `신고 완료` 처리만 동기화하고 접수번호·세액·PDF는 실무 확인 후 확장
  - [x] 개인정보 처리 관계는 수임계약 단계에서 정리하고 매 신고자료 전송에는 반복 동의 대신 수신자·자료 범위 운영 확인만 제공
  - [x] 연결 종료 즉시 미래 전송을 차단하되 기존 수신자료는 계약·법적 의무에 따라 보관하고 삭제 요청은 보관 의무 없는 범위에서 처리
  - [x] 미완료 신고 이벤트가 있으면 연결 종료를 차단하고 신고 완료·외부 수임계약 종료 후 연결 종료하며 비정상 분쟁은 운영자 수동 처리
...
- Acceptance Criteria:
  ...
  - [ ] 전송 때마다 상호명·사업자등록번호로 재탐색하지 않고 수임 승인 시 확정한 고유 연결 ID로 사업자를 라우팅한다.
  - [ ] 기존 동일 사업자가 있으면 새 사업자를 만들지 않으며, 삭제·통합·연결 종료 시 재매핑·재승인 전까지 전송을 차단한다.
  - [ ] 재매핑 이후에도 과거 transfer와 당시 수신 사업자 감사 이력을 보존한다.
  ...
  - [ ] 수신 승인 시 변경 불가능한 전송 snapshot을 `승인된 수신자료`로 승격하고 승인자·시각·transfer ID·snapshot version·fingerprint·이전 version 관계를 보존한다.
  - [ ] 승인된 수신자료를 업무 초안의 입력으로 사용하더라도 확정 장부·신고 정본 승격은 별도 검토·확정 절차를 요구한다.
  - [ ] v1은 같은 목적의 독립된 JARYO 자료와 일반 병합을 전제하지 않고 동일 재시도·보완 재전송만 계약한다.
  - [ ] 하나의 transfer에 부분 수신 상태를 만들지 않고 패키지 전체를 수신 승인·보완 요청·기술 오류 중 하나로 처리한다.
  - [ ] 보완 후에는 새 snapshot version의 전체 패키지를 재전송하고 이전 version 대비 변경 항목을 표시한다.
  - [ ] 동일 transfer ID·contract version·fingerprint 재시도는 기존 수신 결과를 반환하고 중복 수신 건을 만들지 않는다.
  - [ ] 동일 transfer ID의 fingerprint가 바뀌면 기술 오류로 거부한다.
  - [ ] 새 snapshot version 수신 승인 시 이전 승인 version을 `대체됨`으로 보존하고 새 version을 현재 수신자료 정본으로 전환한다.
  - [ ] 수신 승인·보완 요청·기술 오류·재전송 상태가 양쪽에 일관되게 표시된다.
  - [ ] 받은 자료만으로 가능한 세무 판단은 사무소에서 처리하고, 사업자만 알 수 있는 사실·원본이 부족할 때만 보완 요청한다.
  - [ ] 파일 손상·schema·hash·연결 ID 문제는 사업자 보완 요청과 섞지 않고 기술 오류로 분류한다.
  - [ ] 사무소가 값을 변경하면 변경 전·후 값, 사유 유형, 실제 근거 reference, 담당자, 처리시각을 필수로 저장한다.
  - [ ] JARYO AI는 구조화 변경정보와 실제 근거만 사용해 설명 초안을 만들고 사실·증빙·법령 근거를 생성하지 않는다.
  - [ ] AI 작성 설명은 담당자가 검토·수정하고 명시적으로 승인한 뒤에만 SemuAgent로 전송한다.
  - [ ] `SemuAgent 판단 오류` 변경사유는 Ready false positive와 판단 규칙 개선 지표에 연결한다.
  - [ ] SemuAgent는 사무소 최종 조정을 원본·최초 판단과 분리된 변경 불가능한 adjustment로 저장한다.
  - [ ] 전송 상세와 세목 신고 결과 화면은 전송값·사무소 조정값·변경 이유·근거·담당자·시각을 함께 표시한다.
  - [ ] 사무소 조정값을 해당 신고 이벤트의 최종 신고값으로 표시하되 사업자 재승인을 기다리지 않고 알림·문의 경로를 제공한다.
  - [ ] 조정값을 다른 기간·판단 규칙에 자동 확장하지 않으며 재수정은 새 adjustment version으로 이력을 보존한다.
  - [ ] 보완 요청은 대상 transfer/version·원본/거래/판단 reference·예외 원인·질문·필요 자료·담당자·시각을 포함한다.
  - [ ] 보완 요청은 일반 이메일 답변으로 처리하지 않고 SemuAgent의 정확한 구조화 화면 작업으로 연결한다.
  - [ ] 보완 요청 수신 시 관련 신고 이벤트 Ready를 잠그고, 보완 후 전체 gate 재검사와 사업자 확인을 거쳐 새 snapshot을 재전송한다.
  - [ ] `SemuAgent Ready 누락` 보완 요청을 Ready false positive로 집계해 gate·판단 규칙 개선에 연결한다.
  - [ ] 동일 snapshot version·fingerprint·source scope의 자동 재시도는 최초 승인과 같은 idempotency key를 사용하고 중복 수신을 만들지 않는다.
  - [ ] 제한된 자동 재시도 실패 후 `전송 실패`와 수동 `다시 전송` 액션을 제공하며 모든 시도·결과·오류 코드를 감사 기록한다.
  - [ ] 재시도 전 원본·판단·Ready fingerprint가 바뀌면 자동 재시도를 중단하고 전체 Ready 재검사와 사업자 재확인을 요구한다.
  - [ ] JARYO 담당자의 `신고 완료` 처리로 SemuAgent에 `세무사무소 신고 완료`를 표시하고 세목·기간·담당자·완료시각을 자동 기록한다.
  - [ ] v1은 접수번호·최종 세액·접수증 PDF 등록을 필수 업무로 요구하지 않으며 파일럿 실무 확인 전 추정 구현하지 않는다.
  - [ ] 상담 신청 전에는 최소 사업자·연락처 정보만 안내 후 전달하고 수임계약·연결 승인 전 신고자료 전송을 차단한다.
  - [ ] 개인정보 처리 역할·업무 범위·보호조치·필요 계약과 고지는 수임계약 단계에서 정리한다.
  - [ ] 매 전송은 개인정보 반복 동의가 아니라 수신 사무소·세목·기간·자료 범위를 확인하는 운영 승인으로 표시한다.
  - [ ] 개인정보 처리의 정확한 법적 역할과 사용자 문구는 출시 전 개인정보·법무 검토를 통과한다.
  - [ ] 연결 종료 즉시 연결 ID를 비활성화하고 신규·재전송과 사무소의 SemuAgent 새 자료 접근을 차단한다.
  - [ ] 기존 수신자료는 종료된 사업자 업무자료로 분리해 수임계약·법적 의무에 따른 기간 동안 보관한다.
  - [ ] 사업자 삭제 요청은 법적 보관 의무가 없는 자료 범위에서 처리하고 최소 전송·상태 감사 이력은 별도 보존한다.
  - [ ] 정확한 보관기간·삭제 범위·종료 후 접근권한은 출시 전 법무 검토와 파일럿 사무소 실무 확인으로 확정한다.
  - [ ] 검토 대기·검토 중·보완 중인 신고 이벤트가 있으면 일반 연결 종료를 차단하고 완료 후 종료하도록 안내한다.
  - [ ] v1에는 정상 수임 신고를 임의 중단하는 사용자 액션을 만들지 않으며 분쟁·계약 위반·사무소 폐업은 운영자 수동 처리와 감사 기록으로 분리한다.
  - [ ] v1은 수임료·소개비·결제·가격 비교·자동 배정 기능을 제공하지 않는다.
  - [ ] 초기 파트너 파일럿 비용·사업 계약은 제품 밖에서 관리하며 미결정 수익모델을 UI·문서에서 확정된 것처럼 표시하지 않는다.
  - [ ] 파일럿은 첫 수신 승인율·사업자 재요청률·사무소 정리/재입력 시간 감소를 측정해 후속 수익 정책의 근거로 사용한다.
  - [ ] 상담 신청을 JARYO에 즉시 등록하고 최초 이메일 알림을 보내며 사업자에게 신청·사무소 변경 상태를 표시한다.
  - [ ] v1은 상담 응답 SLA·반복 독촉·SemuAgent 운영자 개입·다른 사무소 자동 재배정 기능을 만들지 않는다.
  - [ ] 사무소 응답시간은 파일럿 지표로만 측정하고 제품이 응답 속도를 강제하지 않는다.
