...
- Implementation Preconditions (착수 전 영향 감사 필수):
  ...
  - [x] Slice 4-1 구현 — `createSessionAndSend`·`missing-request`·`period-gap-missing-request`·`missing-request-targets` 모듈 삭제. `session-service.ts`는 `createDirectUploadSession`만 유지. DB migration 없음.
- Acceptance Criteria:
...
- Document Sync Check: Slice 4-0 allowlist + **Slice 4-1(2026-07-06, dead code 제거)** 완료. 다음: **Slice 4-2** upload_session 레거시 컬럼 retirement.
