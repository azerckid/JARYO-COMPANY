...
| JC-030 | todo | 전자신고 검증 및 파일 생성 (Validation / Path 1a·1b) | `lib/efiling-*`, JC-024·013 | **최우선 — Path 1 세목 확대.** 자료대조 Phase 2 완료. **간이지급명세서·원천세·부가세는 공식 양식 미확인 → Path 1b 직접입력 정리 화면 구현·검증 완료.** 간이지급의 과거 고정길이 후보 생성 UI는 제거했다. 다음은 지방소득세 특별징수 Path 1b다. 공식 비암호화 전체 신고 양식이 확인되면 해당 세목만 1a(파일)로 승격한다(`blocked` 세목 없음). Stage A 통과 전 generator를 만들지 않는다. Path 3 암호화 파일은 범위 밖. [Path 1 Roadmap](../03_Technical_Specs/36_PATH1_FORM_FILL_ROADMAP.md) · [VAT Stage A Audit](../03_Technical_Specs/43_JC030_VAT_NONENCRYPTED_UPLOAD_TEMPLATE_AUDIT.md) |
...
- Implementation Preconditions (조사·설계 과제):
  ...
  - [ ] **지방소득세 특별징수** — 확정값 1b 직접입력 정리 화면 구현(미구현); 위택스 공식 규격 확인 시 1a A~G 승격
  - [ ] **사업장현황신고** — 확정값 1b 직접입력 정리 화면 구현(미구현); 면세 개인 대상 공식 업로드 양식 확인 시 1a A~G 승격
  - [ ] **연말 지급명세서** — 확정값 1b 직접입력 정리 화면 구현(미구현); 간이지급과 분리된 별도 공식 레이아웃 확인 시 1a A~G 승격
