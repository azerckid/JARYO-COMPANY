...
- Implementation Preconditions:
  ...
  - [x] 레거시 cron 처분 결정 — vercel.json에서 4개 제거, 라우트 코드 삭제는 후속 chore로 분리
...
- Prototype Review / 승인: N/A - 로드맵 항목. 구현 착수 시 Pre-Code Brief·UI 확인 선행.
...
- Document Sync Check: 2026-07-05 Scope Gate 작성. 입력 가이드(JC-013)와 자동제출(JC-023) 사이의 현실적 다리라는 방향은 유지하되, **바로 구현 금지**: v1 1순위 후보는 근로소득 간이지급명세서이나 최신 파일 레이아웃·직원 식별정보 정책이 미확정. 다음 단계는 UI Preview(파일 생성 패널) 또는 공식 규격/PII 정책 확정 후 Pre-Code Brief.
...
- Acceptance Criteria:
  - [x] 레거시 서브시스템이 단계적으로 제거되며 각 단계에서 tsc/lint/test가 통과한다 (Slice 1c: 207파일 1375건 통과, tsc/eslint/build 클린; Slice 2a: 레거시 요청메일 쓰기 API 12개 410 차단, 회귀 테스트 추가; Slice 2b-1: missing_request runtime side effect 제거, 회귀 테스트 추가; Slice 2b-2: transaction-purpose send 410 차단, 회귀 테스트 추가; Slice 2b-3a: hidden reviews 보충요청 메일 read panel 제거; Slice 2b-3b: hidden emails mail-console read surface 제거; Slice 2b-3c: calendar/client outbound_email read 제거; Slice 2b-3d: dead payroll event loader outbound_email read 제거; Slice 2b-3e: redirect-blocked client event detail/new outbound_email read/UI 제거; Slice 2b-4: stale missing_request draft cleanup side-effect 제거)
  - [x] v1 현행 기능(자료수집·기장·부가세·급여·신고지원·직원명부·리마인드·신고준비)에 영향이 없다 (verifyToken·upload 관련 타깃 15건 + 전체 회귀 통과, 공유 API 미변경; Slice 2a에서 direct-upload·work-email·internal reminder 미변경)
  - [ ] `clients`(사업장)·`billing`·jaryo-admin 등 유지 대상은 보존된다 (Slice 3~4에서 최종 확인)
- Document Sync Check: 2026-07-05 영향 감사 + Slice 1(Route Quarantine) + Slice 1b(죽은 컴포넌트/lib 삭제 + 포털 전용 API 4개 410 차단) + Slice 1c(`session.ts` 포털 조회 dead code 제거) + Slice 2a(레거시 고객 요청메일 쓰기 API 12개 410 차단) + Slice 2b 영향 감사 + Slice 2b-1(`missing_request` runtime side effect 제거 및 `missing-requests` GET 410) + Slice 2b-2(transaction-purpose send 410, `sent_email_id` FK 유지) + Slice 2b-3a(hidden reviews 보충요청 메일 read panel 제거) + Slice 2b-3b(hidden `dashboard/emails` mail-console read surface 제거) + Slice 2b-3c(calendar/client outbound_email read 제거) + Slice 2b-3d(dead payroll event loader outbound_email read 제거) + Slice 2b-3e(redirect-blocked `clients/[id]/events` detail/new read UI 제거) + Slice 2b-4(stale `missing_request` draft cleanup side-effect 제거) 완료. `upload_session`은 현행 source lineage FK라 즉시 삭제 금지 원칙 유지. 공유 API(`/api/upload`·`/api/upload/submit`·`/api/upload/files/[fileId]`+retry/password), direct-upload, work-email, internal reminder는 계속 미변경. `lib/session.ts`는 토큰 검증(`verifyToken`)만 남기고, 포털 렌더링용 `getPortalData`와 하위 헬퍼는 제거. `dashboard/emails`는 redirect leaf로 축소했고, 설정 화면의 업무메일 주소 관리 컴포넌트와 work-email API는 보존. calendar/client 목록, 죽은 payroll event loader, redirect-blocked client event detail/new UI, upload revision/criterion review side-effect는 더 이상 `outbound_email`으로 메일 실패·초안 대기·발송 기록·draft reject 상태를 만들지 않는다. 다음: transaction-purpose 내부 작업 재설계 또는 remaining outbound_email service dead-code 정리, 이후 source lineage 이관 및 schema retirement gate.
