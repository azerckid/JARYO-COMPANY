...
- Document Sync Check: 2026-07-05 영향 감사 + Slice 1~2c(레거시 요청메일·outbound_email 서브시스템 은퇴) + Slice 3(Pre-Code Brief) + Slice 3a(`source_batch` 도입·backfill·dual-write, dev/prod DB 0061 검증 완료) + **Slice 3b(2026-07-06, read model 전환)** + **Slice 3c-0(2026-07-06, downstream FK migration 전략 감사)** 완료. `upload_session`은 Slice 3 기간 동안 compatibility table로 유지, 공유 API(`/api/upload`·`/api/upload/submit`·`/api/upload/files/[fileId]`+retry/password)·direct-upload·work-email·internal reminder는 계속 미변경. Slice 3c 구현 순서는 [Source Batch Replacement Pre-Code Brief §4](../03_Technical_Specs/24_SOURCE_BATCH_REPLACEMENT_PRE_CODE_BRIEF.md)에 고정. 알려진 gap: `getOrCreateFiscalYearLedgerSummary` 자체의 DB 동작 테스트가 이번 변경 전부터 없어 이번에도 신규 작성하지 않음(별도 후속 과제). 다음: Slice 3c-1 company-home read switch(no migration) → Slice 3c-2 source collection validation additive FK → Slice 4 schema retirement 준비.
