...
- External dependencies: Path 1a 파일 지원 세목은 홈택스·국세청의 공식 비암호화 업로드 양식과 직접 수용 경로가 필요하다. 양식이 없는 세목은 Path 1b(직접입력 정리)로 제공한다. **자동 홈택스 제출·자동 납부는 제공하지 않음** — 실제 제출/납부는 회사가 직접 수행.
...
- 원천징수이행상황신고서는 공식 경로가 직접작성 또는 비밀번호 기반 회계프로그램
  변환파일로 확인되어 공식 비암호화 업로드 양식이 없으므로 **Path 1b(직접입력 정리)로
  제공**한다. 확정 A01 집계를 `항목 = 값`으로 보여주고 파일 generator는 만들지 않는다.
- 부가세는 현재 홈택스 회계프로그램 파일변환 메뉴와 일부 첨부서류 도구까지만
  확인했다. 최신 비암호화 수용 여부가 확인되기 전에는 부가세도 **Path 1b로 제공**하고,
  1a 파일 생성 UI는 활성화하지 않는다. Stage A는 1a 승격용 선택 조사다.
