...
- Convenience should come from the table filters, search, row actions, and safe account-pattern batch confirmation. Separate next-action and source-summary card bands are not used.
...
- Period scope must be explicit. The current loader contract exposes month, quarter, and half-year; year and custom range stay hidden until their server query contract exists.
- "증빙없음" is not a completed state. The UI must show action-oriented states such as 증빙 필요, 소명 필요, 소명 완료, 증빙 예외, or 제외됨.

## Current Implementation

Evidence matching and connection, account confirmation, explanation, exclusion, evidence exception, amount-difference blocking, prior-pattern application/rejection, safe account batch confirmation, and shallow undo are implemented. Slice B adds the remaining primary workbench controls: real period navigation, client-side search, and conservative duplicate-suspicion resolution.

## Approved Information Structure

- Period bar: current period label, month/quarter/half-year switch, previous/next period.
- Filter/search bar: source tabs, evidence/explanation/duplicate/exclusion tabs, immediate ledger search.
...
- Required actions: connect evidence, explain usage, confirm evidence exception, resolve duplicate suspicion, apply or reject prior-period pattern recommendation, batch-accept safe repeated account suggestions, assign account, exclude private/business-unrelated use, resolve amount mismatch, and undo the latest action.
...
- Hometax/Wetax entry guidance belongs to each filing screen, not this ledger workbench.
