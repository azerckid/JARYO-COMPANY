...
| 컴포넌트 | 역할 | 상태 |
|:---|:---|:---|
| Readiness Hero | Path 1 양식 생성 전 데이터 준비율·확정·확인 필요·증빙 없음 | 진행률 + 4개 지표 |
| Source Summary Cards | 통장·카드·세금계산서·현금영수증·제외 검토 상태 | source별 건수·미연결·확인 필요 |
| Source Tabs | 전체/통장/카드/세금계산서/현금영수증/증빙 없음/제외 검토 | 세그먼트 탭 + 건수 |
| Reconciliation Ledger Table | 출처별 거래 후보를 하나의 신고 전 원장으로 대조 | 거래일·출처·거래처·적요·공급가액·세액·증빙·계정·상태·처리 |
| Evidence Link Status | 통장↔카드↔세금계산서↔현금영수증 연결 상태 | 연결됨/연결 없음/금액 차이/카드 합계 차이 |
...
| Exclusion Review | 사적 사용·업무무관·중복 후보 처리 | 업무사용/제외/메모 |
...
- 전월/최근 확정 패턴은 추천 근거로만 사용한다. 같은 사업장 안에서 반복 거래의 계정·증빙·제외 후보를 설명하지만, 사용자의 확인 없이 자동 확정하지 않는다.
