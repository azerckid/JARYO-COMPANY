...
진입: 사이드바 "신고 준비". 회사가 직접 신고하기 전에 홈택스·위택스에 넣을 확정 데이터가 준비됐는지 한 화면에서 확인한다.
연계: 공통 기반(자료수집·기장검토)에서 만들어진 확정 거래원장을 원천세·부가세·지급명세서/연말정산·지방소득세 병렬 트랙으로 넘기고, 각 트랙의 끝은 신고지원 handoff로 연결한다.
...
- 중간 2 = 병렬 신고 트랙(원천세·부가세·지급명세서/연말정산·지방소득세)의 입력·산출·handoff
...
핵심 경계: 이 화면은 **신고 준비 데이터**를 정리·검토하는 read-only 집계 화면이다. 정산액(결정세액·환급/추징) 계산, 전자신고 파일 생성(JC-030), 홈택스 제출은 이번 범위 밖이며 최종 제출은 사용자가 직접 수행한다. 자동 제출·자격증명 저장은 하지 않는다.
...
- 확인 방식: 브라우저에서 HTML 파일 직접 열람. 기존 6개 워크스페이스와 직원 명부·리마인드·신고 준비는 사이드바/CTA로 상호 이동 가능.
