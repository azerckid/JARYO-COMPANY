...
- 승인일: 2026-07-11
- 현재 구현: VAI-3a 공식 규칙·이전 확정 패턴, VAI-3b single-provider AI 보강, VAI-4a/4b 사용자 확정·되돌리기에 이어 VAI-5가 영세율·면세·1천만원 이상·낮은 신뢰도 행만 Gemini·OpenAI 합의와 조건부 Claude 중재로 보강한다. exact VAT fact가 없는 행은 억지로 추정하지 않고, 영세율·면세 필수 증빙이 없으면 확정을 차단한다.
- 확인 경로: `/dashboard/vat?period=2026-H1`의 영세율·불공제·안분·공제·면세 대표 행에서 액션 버튼, 판단 근거 대화상자, 최근 작업 되돌리기를 확인한다.
- 다음 작업: VAI-5 오너 확인 뒤 VAI-6에서 사용자 확정 결과만 VAT rebuild/package gate가 소비하도록 연결하고 JC-035를 마감한다.
