...
- 매출 구분 3카드는 항상 표시한다. AI 판단과 매입세액 공제 검토는 `신고 전 수정 필요` 한 작업대로 통합한다.
...
- 수정 대상이 0건이면 작업대 전체를 숨기고 Hero 한 줄만 유지한다.
...
- 세액 요약 바로 아래에 `추가 공제 가능성`을 두고 `신고 전 수정 필요`와 분리한다.
...
### 6.5 JC-038 VUI-1d Final Simplification — Owner Approved

- `추가 공제 가능성`을 부가세 화면의 절세 핵심 영역으로 유지한다.
- 기존 일반 예외 큐는 `신고 전 수정 필요`로 좁힌다. `expected_no_change` 행은 AI 출처이거나 미확정이라는 이유만으로 표시하지 않는다.
- 홈택스 수정 행동·unresolved handoff·pending 공제 검토만 표시하며 0건이면 섹션 전체를 렌더하지 않는다.
