...
> **구현 현황(2026-07-11):** 이 절은 제품 모델(목표)을 정의한다. 현재 1a 파일은
> 근로소득 간이지급명세서만 구현됐고, 원천세·부가세는 1b 대상으로 **결정**됐으나
> **1b 값 정리 화면은 아직 구현하지 않았다**(현재 앱은 준비값·검증 패널만 표시).
...
- **1b 직접입력 정리**: 공식 업로드 양식이 없거나 암호화·인증만 요구되면, SemuAgent가
  ...
  입력하게 한다. **값 정리 표시까지가 범위**이며, 홈택스 메뉴·입력칸 위치를 단계별로
  안내하지는 않는다.
...
**검증**한다 (`lib/efiling-simplified-wage` 등). 이 레이어는 Path 1과 향후 Path 2가 공유하는 품질 관문이다.
...
|---|---|---|
| 1 | **최우선** — 세목별 1a(양식 업로드) 우선, 없으면 1b(직접입력 정리) | 세목 확대: 양식 확인되면 1a, 아니면 1b 대상으로 순차 전환(원천세·부가세 포함 · 1b 값 정리 화면은 구현 대기) ([Path 1 Roadmap](../03_Technical_Specs/36_PATH1_FORM_FILL_ROADMAP.md)) |
