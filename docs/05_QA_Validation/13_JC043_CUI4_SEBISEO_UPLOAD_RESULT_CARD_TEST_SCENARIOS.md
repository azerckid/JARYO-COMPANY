...
| R-05 | 카드·표 status 정합 | 최근 세션 CTA landing 후 행 status | `needs_review`/`matched` 건수가 카드 버킷과 일치 | **PASS · Preview browser (2026-07-18, S1 `needs_review` 1건)** |
| R-06 | tenant 격리 | tenant B `sessionId`를 tenant A CTA에 수동 주입 | redirect로 `sessionId` strip. tenant B 파일 **0건** 노출 | **PASS · Preview browser (2026-07-18)** |
| R-07 | 사업장 격리 | 다른 `clientId` sessionId 주입 | §4.3.3 redirect. 타 사업장 파일 0건 | **PASS · Preview browser (2026-07-18)** |
...
- R-04·R-09 계약 로직은 `lib/source-collection/session-deep-link.ts` 단위 테스트로 고정한다. 로그인 세션이 필요한 브라우저 확인은 staging/Preview에서 별도 수행한다.
- 2026-07-18 초기 Preview browser evidence는 로그인된 샘플 회사에서 실행했다. 단일 샘플 화면만으로는 R-04·R-06·R-07을 통과 처리하지 않았고, 후속 fixture 검증으로 보완했다. R-11은 새 업로드와 **개별 fixture 정리 경로**를 먼저 갖춘 뒤 실행한다.
- R-12는 같은 Preview에서 카드의 CTA와 자료수집 landing을 확인했다. 확인·제출·재분석 제어는 없고, 표시된 `다시 업로드` 링크는 기간 전체 자료수집 화면으로 이동한다.
