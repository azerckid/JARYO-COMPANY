...
| R-04 | **같은 기간 다른 세션 혼입 없음** | (1) period P에 session S1(파일 A), S2(파일 B) 각각 업로드 (2) S1이 카드에 표시될 때 CTA 클릭 (3) 가져오기 상태 표 행 확인 | 표에 **S1 소속 파일만** 표시. S2 파일 B **미표시**. 행 수 = 카드 `totalCount` | Pending |
