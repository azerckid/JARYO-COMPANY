...
UI-First 진행: 부가세 1b — 법정 신고서 행·칸 Field Mapping과 별도 `홈택스 입력값` Preview 제작 완료, 프로젝트 오너 확인 대기
다음 제품 작업: Preview 승인 → 부가세 1b read model·runtime 화면 구현 → 지방소득세 특별징수 Stage A
...
| 2 | **부가가치세** | 자료대조 Phase 2·VAT deduction·provenance 완료 | 현재 홈택스 `파일 변환신고(회계프로그램)`와 2020년 일부 첨부서류 Excel 도구 확인; 전체 신고 비암호화 양식 미확인 | 1a 파일 없음 · 1b Field Mapping·Preview 완료, runtime 미구현 | **1b UI-First Gate 오너 확인 대기; 1a 양식은 Stage A 확인 시** |
...
### 4.3 VAT — 1b 직접입력 정리(UI-First 진행) + 1a 양식(Stage A 확인 시)

부가세는 공식 비암호화 전체 신고 양식이 아직 확인되지 않아 **기본 1b(직접입력
정리) 대상으로 판정**한다. 법정 신고서 행·칸 Field Mapping과 별도 HTML Preview는
제작했으며 프로젝트 오너 확인과 runtime 구현은 남아 있다. 1a 파일 트랙은 Stage A
외부 확인이 통과할 때만 추가한다.
...
2. **1b UI-First Gate (진행)** — [VAT Path 1b Preview](../02_UI_Screens/previews/14_vat_path1b.html)를 별도 화면으로 제작했다. 프로젝트 오너가 표 구조·문구·책임 경계를 확인하기 전 runtime을 구현하지 않는다.
3. **1b Runtime (대기)** — Preview 승인 후 tenant·사업장·기간 scoped read model과 `홈택스 입력값` 화면을 구현한다. 파일 generator는 만들지 않는다.
