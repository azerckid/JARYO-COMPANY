...
다음 제품 작업(미구현): 원천세·부가세 1b 직접입력 정리 화면 구현 → 지방소득세 특별징수 Stage A
...
1. 근로소득 간이지급명세서가 §2.1(1a 파일)을 통과한다. 나머지 세목은 1a 양식이
   확인되면 1a로, 아니면 1b(직접입력 정리) 대상으로 두고 값 정리 화면을 구현한다.
2. 1a 세목은 대표 데이터로 양식 채움 확인 → 파일 다운로드 → 홈택스 비암호화 업로드
   검증까지, 1b 세목은 `항목 = 값` 정리 화면 확인까지 완료한다.
3. beta 사용자가 실패 원인과 수정 위치를 화면에서 확인할 수 있다.
4. 자동제출, Path 2, 암호화 파일, 단계별 위치 안내 없이 Path 1(1a/1b)로 검증한다.
...
| 1 | **원천세 신고서** | JC-012·JC-013 | W0 공식 조사 완료; 공식 업로드 양식 없음(직접작성/비밀번호 변환파일만) | 1a 파일 없음 · 1b 값 정리 **대상(화면 미구현)** | **1b 직접입력 정리 대상 · 구현 대기 (1a 양식 미확인)** |
...
| **1b** | 확정 A01 집계를 `항목 = 값`으로 직접입력 정리 화면 구현 | **미구현(대상 판정만 완료)** — 완료선: 기존 검증 패널 + 값 정리 화면; 파일 generator 없음 |

원천세는 [공식 근거 감사](./37_JC030_WITHHOLDING_EFILING_LAYOUT_ACQUISITION.md)상
공식 비암호화 업로드 양식이 확인되지 않아 **1b(직접입력 정리) 대상으로 판정**했다.
**1b 값 정리 화면은 아직 구현하지 않았고**(현재 앱은 검증 패널만 표시), 화면 구현은
후속 작업이다. 최신 공식 양식과 직접 수용 메뉴가 새로 확인되면 그때 1a(파일 W1~W5)로 승격한다.
