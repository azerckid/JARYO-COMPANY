...
구현 완료·검증 중: 부가세 1b — 법정 신고서 행·칸 Mapping, 승인 Preview, Pre-Code Brief, scoped read model, 별도 `홈택스 입력값` runtime
다음 제품 작업: 부가세 1b browser/전체 회귀 검증 → 지방소득세 특별징수 Stage A
...
3. **1b Pre-Code Brief·Runtime (구현 완료·검증 중)** — tenant·사업장·기간 scoped 확정 VAT fact와 package/provenance gate를 재사용해 `홈택스 입력값` 별도 화면을 구현했다. 미완료 상태에서는 값을 숨기고, `(27)`은 홈택스 최종 확인으로 남긴다. 파일 generator는 만들지 않는다.
