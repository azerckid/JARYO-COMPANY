...
UI-First 완료: 부가세 1b — 법정 신고서 행·칸 Field Mapping과 별도 `홈택스 입력값` Preview 프로젝트 오너 승인(2026-07-13)
다음 제품 작업: Pre-Code Brief → 부가세 1b read model·runtime 화면 구현 → 지방소득세 특별징수 Stage A
...
| 2 | **부가가치세** | 자료대조 Phase 2·VAT deduction·provenance 완료 | 현재 홈택스 `파일 변환신고(회계프로그램)`와 2020년 일부 첨부서류 Excel 도구 확인; 전체 신고 비암호화 양식 미확인 | 1a 파일 없음 · 1b Field Mapping·Preview 오너 승인 완료, runtime 미구현 | **1b Pre-Code Brief·runtime 대기; 1a 양식은 Stage A 확인 시** |
...
제작하고 프로젝트 오너 승인을 완료했으며 Pre-Code Brief와 runtime 구현은 남아 있다. 1a 파일 트랙은 Stage A
외부 확인이 통과할 때만 추가한다.
...
3. **1b Pre-Code Brief·Runtime (대기)** — tenant·사업장·기간 scoped read model과 `홈택스 입력값` 화면 계약을 고정하고 구현한다. 파일 generator는 만들지 않는다.
