...
> Last Updated: 2026-07-05 14:30

## 0. Flow Status

```text
[Flow]
현재: JC-031 — Impact Audit / Retirement Plan
Gate: 진행 중
완료: 레거시 cron 라우트 삭제, 신고 준비 허브 전 트랙 live, JC-030 scope gate
다음: 외부 포털/요청메일 레거시 경계 승인 → 단계별 삭제 PR
필요 확인: /upload/[token] 외부 포털 v1 제외 최종 승인, upload_session 대체 source batch 모델 설계
...
- adaptive structure model/run 계열
...
주의: `transaction_purpose_request.sent_email_id -> outbound_email.id`가 남아 있으므로,
메일 테이블 삭제 전 transaction purpose 확인 요청 흐름을 내부 self-use 방식으로 재설계해야 한다.
...
- `outbound_email` 신규 생성 경로 차단
- request-event 기반 발송 API 제거 또는 410 처리
- transaction purpose request의 메일 발송 경로를 내부 확인 작업으로 재설계하거나 후속 보류
...
권장 1순위는 **외부 포털 `/upload/[token]` v1 제외 승인 후 quarantine**이다. 제품 경계가 가장 명확하고,
DB FK 이관 없이도 SemuAgent self-use 방향과 맞지 않는 공개 표면을 먼저 닫을 수 있다.
