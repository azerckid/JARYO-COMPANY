...
| 원천세 신고서 | 급여/원천 데이터 live, 지방소득세 정합성까지 완료, 월 신고 | 공식 업로드 양식 없음(직접작성/비밀번호 변환파일만) | **1b 직접입력 정리 대상 · 화면 구현 대기** |
| 부가가치세 | SemuAgent의 핵심 세목, 직원 PII 의존 낮음, Phase 2 gate/provenance 완료 | 전체 신고 비암호화 양식 미확인(회계프로그램 변환·일부 첨부서류만) | **1b 직접입력 정리 대상 · 화면 구현 대기; 1a는 Stage A 확인 시** |
...
**다음 확대:** 원천세·부가세는 1b(직접입력 정리) 대상으로 결정됐고 값 정리 화면 구현이 다음 작업이다. 이후 지방소득세 등 각 세목 1a 없으면 1b 대상. 순서·게이트는 [Path 1 Form Fill Roadmap](./36_PATH1_FORM_FILL_ROADMAP.md).
...
우회하지 않고 **1b(직접입력 정리) 대상으로 둔다**(값 정리 화면은 별도 구현 과제). 세목을 `blocked`로 두지 않는다.
