...
구현 완료: 근로소득 간이지급 — Validation + 비암호화 파일 후보·검증·홈택스 안내; 공식 직접 수용은 beta 검증 대상
차단 종료: 원천세 W0 — 공식 경로가 직접작성 또는 비밀번호 기반 변환파일이라 Path 1 부적합
현재: 부가세 Stage A — [공개 자료 감사 완료, 로그인 화면·126 외부 확인 대기](./43_JC030_VAT_NONENCRYPTED_UPLOAD_TEMPLATE_AUDIT.md)
...
| 원천세 신고서 | 급여/원천 데이터 live, 지방소득세 정합성까지 완료, 월 신고 | 공식 안내가 직접작성 또는 비밀번호 기반 회계프로그램 변환파일 | **W0 closed blocked** |
| 부가가치세 | SemuAgent의 핵심 세목, 직원 PII 의존 낮음, Phase 2 gate/provenance 완료 | 현재 회계프로그램 파일변환 메뉴·일부 첨부서류 도구 확인; 최신 전체 신고 비암호화 규격·수용 미확인 | **Stage A 공개 감사 완료, 외부 확인 대기 · partial / blocked** |
...
**다음 확대:** 원천세 W0 차단 종료 → 부가가치세 Stage A. 순서·게이트는 [Path 1 Form Fill Roadmap](./36_PATH1_FORM_FILL_ROADMAP.md).
