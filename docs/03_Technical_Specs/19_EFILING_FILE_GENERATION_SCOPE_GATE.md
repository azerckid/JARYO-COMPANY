...
1b 전환: 원천세 — 공식 업로드 양식 없음(직접작성/비밀번호 변환파일만) → 직접입력 정리로 제공
1b 전환: 부가세 — 전체 신고 비암호화 양식 미확인 → 직접입력 정리로 제공; 1a는 [Stage A 확인](./43_JC030_VAT_NONENCRYPTED_UPLOAD_TEMPLATE_AUDIT.md) 시
보류: JC-034 Path 2 ZIP 구현
제외: Path 3 fcrypt·암호화 파일·적합성 검정, 단계별 위치 안내
```
...
| 후보 | 장점 | 차단/리스크 | 판정 |
|---|---|---|---|
| 근로소득 간이지급명세서 | JC-024 데이터가 이미 live, 공식 제출주기 확인, 소규모 회사 빈도 높음 | 대표 파일의 공식 비암호화 직접 업로드 수용 재검증 필요 | **구현 완료, beta 검증 대상** |
| 원천세 신고서 | 급여/원천 데이터 live, 지방소득세 정합성까지 완료, 월 신고 | 공식 업로드 양식 없음(직접작성/비밀번호 변환파일만) | **1b 직접입력 정리 지원** |
| 부가가치세 | SemuAgent의 핵심 세목, 직원 PII 의존 낮음, Phase 2 gate/provenance 완료 | 전체 신고 비암호화 양식 미확인(회계프로그램 변환·일부 첨부서류만) | **1b 직접입력 정리 제공; 1a는 Stage A 확인 시** |
| 법인세/종합소득세 | 커버리지 확장 가치 큼 | JC-025/026 미구현, 법무/세무조정 리스크 큼 | 후순위 |

## 4. v1 Direction

JC-030 **1차 구현 세목:** 근로소득 **간이지급명세서** (공식 직접 업로드 수용은 beta 검증 대상).

**다음 확대:** 원천세·부가세는 1b(직접입력 정리)로 제공, 이후 지방소득세 등 각 세목 1a 없으면 1b. 순서·게이트는 [Path 1 Form Fill Roadmap](./36_PATH1_FORM_FILL_ROADMAP.md).
...
아니다. Path 1 세목이 공식 비암호화 업로드 양식(1a)을 지원하지 않으면 암호화 경로로
우회하지 않고 **1b(직접입력 정리)로 제공**한다. 세목을 `blocked`로 두지 않는다.
