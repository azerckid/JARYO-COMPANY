...
3. **결과 카드 UI (Preview 정합)**
   - 라벨: `{periodLabel} · 직접 업로드` — `accountingPeriod`를 기존 period 헬퍼로 사람이 읽는 라벨로 변환.
...
4. **CTA (navigation only)**
   ...
   - `periodKey`는 해당 세션의 `accountingPeriod`에서 역산(자료수집 `loadSourceCollectionSummary`와 동일 헬퍼).
   - 라벨 규칙:
...
4) upload_file WHERE upload_session_id = session.id AND tenant_id
5) status별 count → SebiseoUploadResultCard | null
```
...
| period 역산 | `accountingPeriod` → `periodKey` 변환은 기존 헬퍼만 사용. 임의 문자열 조합 금지 |
...
| period 정합 | query `periodKey`가 세션 `accountingPeriod`에서 역산한 key와 **일치** | §4.3.3 |
...
| `lib/sebiseo/period-options` | 업로드 기간 확인(기존) |
...
| Slice | 산출물 | 완료선 |
|:---|:---|:---|
| **CUI-4a** | `loadSebiseoUploadResultCard` + Zod + unit test + 카드 UI + page prop + §4.3 import 표 세션 필터 | tenant/사업장/기간 격리. 세션 없으면 미표시. CTA landing 시 표 혼입 0 |
