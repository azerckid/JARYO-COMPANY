...
| 공식 레이아웃 묶음 | `지급명세서 전산매체 제출요령`(홈택스 1175 등) | `간이지급명세서(근로소득) 전산매체 제출요령` | **`전자신고 이용안내` 원천세 장 + 홈택스 원천세 변환프로그램 번들** [Slice 0b: 별도 HWP 미발견] |
...
## 3. Official Acquisition Path (Slice 0b — 2026-07-07 조사 결과)

### 3.0 Slice 0b 판정

| 조사 | 결과 |
|------|------|
| 홈택스 자료실·NTS 프로그램 게시판 검색 (`원천징수이행상황` + `전산매체`) | **별도 `…전산매체 제출요령` HWP 미발견** [확실·2026-07-07] |
| 지급명세서(1175)·간이지급(SC) HWP | **다른 문서** — 혼용 금지 [확실] |
| 서식·작성 절차 | law.go.kr **별지 제21호**(2024.3.22) · NTS **작성요령**(2009, `nttSn=74837`) — **서식 필드** 참조용 |
| 바이너리 레이아웃 | **전자신고 이용안내**(고시 §12) 또는 홈택스 **원천세 변환프로그램** 설치 번들 [추정·관문] |

로컬 참조: `scratch/jc-030-reference/withholding/` (gitignored).

### 3.1 1순위 — 홈택스 변환프로그램 번들

- 경로: 홈택스 → **프로그램 설치** → 세금신고 → **원천세** [다운로드]
- 기대 산출물: 변환프로그램 + **동봉 레이아웃/이용안내** (wrjb·실무 SW: "프로그램에 대한 설명도 함께 내려받을 수 있음")
- 제출 관문: 바탕화면 **[원천세 변환]** → 오류검증 → 홈택스 전송 ([NTS 작성요령 §3-4](./37_JC030_WITHHOLDING_EFILING_LAYOUT_ACQUISITION.md))
...
JC-013 신고지원은 동일 세목에 대해 **신고 준비값 확인·접수증 보관**을 제공한다.
JC-030 Path 1은 **변환제출용 파일 생성**을 담당한다.

홈택스 화면에 값을 옮겨 적는 방식은 SemuAgent 범위가 아니다. 이 문서는 Path 1 업로드용 파일·양식 작성 범위만 다룬다.
...
- [x] Slice 0b 참조 PDF 입수 (별지 제21호·NTS 작성요령) — **서식 필드**
- [x] Field Mapping Part A 초안 — [38](./38_JC030_WITHHOLDING_EFILING_FIELD_MAPPING.md)
- [x] Pre-Code Brief 초안 — [39](./39_JC030_WITHHOLDING_EFILING_PRE_CODE_BRIEF.md)
- [ ] **바이너리 레이아웃** (`전자신고 이용안내` / 변환프로그램 번들)
- [ ] 파일명 접두·record type·인코딩 확정
- [x] PII 범위 v1 — **A01 집계-only**, 직원 부표·주민번호 제외
