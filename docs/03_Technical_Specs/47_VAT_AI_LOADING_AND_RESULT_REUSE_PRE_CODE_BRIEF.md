...
JC-037을 JC-039보다 먼저 구현하더라도 결과 저장소가 `recommendation: needs_review` 하나에
판단과 workflow를 고정하면 안 된다. additive schema/Zod는
[JC-039 VAI-8](./50_VAT_AI_EVIDENCE_BACKED_DECISIVE_JUDGMENT_BRIEF.md)의
`provisionalJudgment`, `workflowStatus`, `evidenceTrace`를 후속 migration 없이
수용하거나, 명시적으로 versioned payload를 교체할 수 있어야 한다.
...
- [ ] 사용자 확정 행은 자동 provider 호출 대상에서 제외된다. (VAI-7b service guard PASS, VAI-7c trigger 경계 Pending)
