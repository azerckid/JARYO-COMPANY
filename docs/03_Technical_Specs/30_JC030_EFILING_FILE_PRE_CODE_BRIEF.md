...
- 슬라이스 2a의 plain 파일은 **암호화 전 사전검증 후보**이며, 홈택스 제출 가능 파일이라고 보장하지 않는다.
...
7. fcrypt 암호화 래퍼(8~15자리 사용자 비밀번호) — **슬라이스 2b**; 슬라이스 1·2a는 plain + 검증만
...
| Request body (Zod) | 주민번호·B8 주민/법인번호·세무서코드·담당자 연락처·암호(8~15) |
...
| 암호화 전 plain 파일 후보 | plain 파일만으로 제출 가능 |
...
5. **검증·다운로드** — 사전검증 칩 · plain 후보 파일 다운로드 · 홈택스 업로드 안내 링크
...
- [ ] 사용자가 plain 후보 파일을 다운로드할 수 있으나, 앱은 제출 가능 파일이라고 보장하지 않음
...
4. Staging에서 홈택스 변환제출 **시험 검증**(사용자 수동) — plain 후보/암호화 파일 수용 여부 확인
