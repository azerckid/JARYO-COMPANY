...
| `provenance_version`, `source_fingerprint`, `source_row_count`, `rebuilt_at` | 확정 VAT fact deterministic rebuild 계약·입력 fingerprint·행 수·재계산 시각. 최초 rebuild 전에는 null이며, 이후 현재 입력이 fingerprint와 다르면 package gate가 다시 잠김 |
...
VAT fact를 보존하며, Slice 2d-3c의 `lib/vat/provenance.ts`가 확정 fact와 같은
scope의 공제 검토만으로 summary를 deterministic rebuild한다. 현재 입력으로 다시
계산한 fingerprint와 저장 metadata·금액이 모두 일치할 때에만 package provenance를
통과시킨다.
...
- 구현 완료: migration `0067_add_vat_fact_provenance.sql`의 exact VAT fact/provenance 필드와 Slice 2d-3c deterministic rebuild·fingerprint 검증을 연결했다. 기존/sample 행은 추정 backfill하지 않으며 exact source basis가 없으면 잠금 상태를 유지한다.
