...
- **신규 세액 계산 없음.** 실제 저장값을 합산할 뿐, 10%/11 같은 근사 계산을 도입하지 않는다.
- **범위**: 종합소득세분·법인세분 지방소득세는 JC-025/026 완료 이후 별도. 위택스 자동 제출은 JC-023 게이트 전 미도입.
- **데이터 정합성 수정(같은 범위)**: 신고지원(JC-013)이 `splitWithholdingTax()`(원천세 합계를 10%/11로 근사 분리)로 지방소득세를 표시하던 것을, 이 화면과 **같은 실제값 소스**로 교체한다. 두 화면이 같은 기간에 다른 숫자를 보이지 않는다.
...
4. **신고지원(JC-013) 정합성 수정** — `splitWithholdingTax()` 사용을 실제 `incomeTaxKrw`·`localIncomeTaxKrw` 합계로 교체
...
- **집계는 상태 무관하게 전체 라인 합산**한다 — `payrollPeriodSummary.withholdingTaxKrw`가 이미 이 규칙(`buildPayrollSummaryTotals`, 상태 무관 합산 + `needs_review`는 별도 `issueCount`)을 따르므로 동일하게 맞춘다. `needs_review` 라인은 합계에 포함하되 "확인 필요" 라벨로 별도 표시한다.
- 이 로더는 **JC-027 화면과 신고지원(JC-013) 양쪽에서 재사용**한다 — 두 곳이 서로 다른 계산을 하지 않도록 단일 소스로 둔다.
...
type LocalIncomeTaxSummary = {
  ...
  hero: {
    ...
    localIncomeTaxTotalKrw: number  // Σ localIncomeTaxKrw (상태 무관 전체 합산)
  }
...
- Hero의 `localIncomeTaxTotalKrw`와 화면 표의 합계 행은 **반드시 같은 소스(§2 로더)에서 계산**해 Preview에서 지적된 것과 같은 숫자 불일치가 재발하지 않게 한다(단위 테스트로 고정, §6).
...
**변경 후**: `buildFilingInputGuide`가 §2의 공유 로더로 얻은 실제 `incomeTaxKrw`·`localIncomeTaxKrw` 합계를 직접 받는다(파라미터로 전달). `splitWithholdingTax()`는 이 교체 이후 호출부가 없으면 **삭제**한다(사용처·테스트 확인 후 dead code 정리 — YAGNI, 백업용으로 남기지 않는다).

- `loadFilingSupportSummary`가 `payrollPeriodSummary`를 조회해 `periodSummaryId`를 얻는 지점(기존 쿼리, `summary.ts:655` 부근)에서 §2의 `loadLocalIncomeTaxLines`(또는 그 합산 결과)를 함께 로드해 `PayrollFilingSource`에 `incomeTaxKrw`·`localIncomeTaxKrw` 필드를 추가한다.
...
- [ ] 원천세 특별징수분 지방소득세가 직원별로 집계·표시된다(실제 `localIncomeTaxKrw` 합계, 파생 계산 아님)
- [ ] Hero 합계와 표의 합계 행이 항상 같은 값이다(같은 로더 재사용, 단위 테스트로 고정)
- [ ] 신고 준비 허브의 `local_income` 트랙이 roadmap→live로 전환되고 검토 화면으로 링크된다
- [ ] 신고지원(JC-013)의 지방소득세 표시가 `splitWithholdingTax` 대신 실제 합계를 사용한다(정합성) — 신고지원과 JC-027 화면이 같은 기간에 같은 숫자를 보인다
