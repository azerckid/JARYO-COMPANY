...
결과 카드(실DB 건수·필터 CTA) = [CUI-4 Brief 63](./63_JC043_CUI4_SEBISEO_UPLOAD_RESULT_CARD_PRE_CODE_BRIEF.md)
...
정본은 `lib/upload/allowed-content-types.ts`이며 `app/api/upload/route.ts`와 클라이언트가 함께 사용한다.
...
- [x] Concept 04 Status → CUI-3a 완료 / CUI-3b 구현·검증 진행
- [x] BACKLOG JC-043 Related Technical/QA Docs 링크 유지
- [x] SCREEN_FLOW §2.1 CUI-3b 범위 문구
- [x] Preview `19_sebiseo.html`에 활성 첨부·입력·기간 확인(Instant 등은 disabled) 반영
