...
| 2b-2c-3 | 2b | Amount mismatch resolution | G: amount mismatch; P: resolve amount mismatch | User can resolve partial/different amount cases from the evidence modal. Remaining difference and selected total must update before save; full split/merge editor remains out of scope unless separately briefed. |
...
- [x] Slice 2b-2c-2 evidence exception implemented — v1 saves `증빙 예외: ...` in `staffMemo`, clears any saved evidence link, and renders the row as `증빙 예외` without reopening the evidence finder.
