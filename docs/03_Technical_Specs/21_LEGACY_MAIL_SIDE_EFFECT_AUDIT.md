...
> Last Updated: 2026-07-05 17:27

## 0. Flow Status

```text
[Flow]
현재: JC-031 Slice 2b-3a 완료 — hidden reviews mail read cleanup
Gate: 통과
완료: Slice 1~1c 외부 포털 quarantine, Slice 2a 레거시 요청메일 쓰기 API 410 차단, Slice 2b 영향 감사, Slice 2b-1 보충요청 초안 생성 side effect 제거, Slice 2b-2 transaction-purpose send quarantine, Slice 2b-3a hidden reviews approval queue read 제거
다음: Slice 2b-3b dashboard/emails mail-console cleanup 또는 calendar/client outbound-email read 영향 분리
...
- `app/(dashboard)/dashboard/emails/*` — `emails/layout.tsx` redirect로 차단된 레거시 메일 화면. Slice 2b-3b 정리 후보.
...
3. **Slice 2b-3 — Legacy mail UI/read cleanup**
   ...
   - **2b-3b 후보:** `dashboard/emails` mail-console read surface 정리.
