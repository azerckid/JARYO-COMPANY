...
export async function saveReconciliationRowEvidenceException(params: {
  uploadSessionId: string
  rowId: string
  memo: string
}): Promise<ReconciliationRowMutationResult> {
  return patchClassificationRow({
    uploadSessionId: params.uploadSessionId,
    rowId: params.rowId,
    body: { staffMemo: params.memo, linkedEvidenceRowId: null },
    fallbackErrorMessage: '증빙 예외 처리에 실패했습니다.',
  })
}
...
export async function confirmReconciliationRowAsDistinct(params: {
  uploadSessionId: string
  rowId: string
  memo: string
}): Promise<ReconciliationRowMutationResult> {
  return patchClassificationRow({
    ...
    rowId: params.rowId,
    body: { staffMemo: params.memo },
    fallbackErrorMessage: '별도 거래 확인에 실패했습니다.',
  })
}
