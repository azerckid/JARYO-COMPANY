...
describe('bookkeeping review UI boundaries (JC-010)', () => {
  ...
  it('renders the reconciliation ledger route as a dedicated Path 1 gate', () => {
    ...
    expect(reconciliationViewSource).toContain('placeholder="거래처, 금액, 적요 검색"')
    expect(reconciliationViewSource).toContain('filter="duplicate_review"')
    expect(reconciliationViewSource).toContain('ReconciliationDuplicateReviewModal')
    expect(reconciliationViewSource).toContain('activePeriodLabel')
    expect(reconciliationViewSource).not.toContain('Path 1 데이터 준비 관문')
    expect(reconciliationViewSource).not.toContain('표시 설정')
    expect(reconciliationViewSource).not.toContain('선택 확정')
